    ...
    <div class="detail-shell">
      <header class="detail-header">
        <span class="detail-header__eyebrow">
          {{ pokemon?.subFormatName || 'Pokémon' }}
        </span>

        <h1 class="detail-header__title">
          {{ $t('pokemonsSeccion.detail') }}
        </h1>

        <p class="detail-header__subtitle">
          {{
            pokemon
              ? `Información competitiva de ${pokemonDisplayName}.`
              : 'Consulta los datos competitivos del Pokémon.'
          }}
        </p>
      </header>
...
export default {
  ...
  computed: {
    ...
    shareText() {
      return `Pokémon ${this.pokemonDisplayName}`;
    },
  ...
  methods: {
    ...
    async getPokeDetail(trackEvent = true) {
      ...
      if (!pokemonId) {
        ...
        this.errorMessage = 'El identificador del Pokémon no es válido.';
        return;
      }
      ...
      try {
        ...
        const rawPokemon = Array.isArray(responseData)
          ? responseData[0]
          : responseData?.data?.[0] ||
            responseData?.data ||
            null;
